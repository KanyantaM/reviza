...
class EditMyCoursesBloc extends Bloc<EditMyCoursesEvent, EditMyCoursesState> {
  EditMyCoursesBloc(
      ...
      : _studentOfflineDataRepository = studentOffline,
        _studentOnlineDataRepository = studentOnline, super(EditMyCoursesInitial()) {
    on<FetchMyCourses>((event, emit) async{
      emit(FetchingCoursesState());
      try {
       Student currentStudent = /*await _studentOfflineDataRepository.getUserById(event.studentId) ??*/ Student(userId: event.studentId, myCourses: []);
      //  if (currentStudent.myCourses.isEmpty) {
         currentStudent = await _studentOnlineDataRepository.getUserById(event.studentId) ?? Student(userId: event.studentId, myCourses: []);
      //  }
    ...
    on<AddMyCourse>((event, emit) async{
      try {
